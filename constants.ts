...
export const SKILLS_DB: Record<string, Skill> = {
  ...
  // Scout
  FLANKING_SHOT: { id: 'FLANKING_SHOT', name: 'Flanking Shot', description: 'Bonus Dmg if enemy distracted.', type: 'ATTACK', damageType: 'KINETIC', animation: 'RAILGUN', minLevel: 4, synergy: 'DISTRACTED' },
  FLASHBANG: { id: 'FLASHBANG', name: 'Flashbang', description: 'Low Dmg + Weaken.', type: 'SUPPORT', damageType: 'THERMAL', animation: 'EXPLOSION', minLevel: 2, statusEffect: 'WEAKEN' },

  // Assault
  SUPPRESSION: { id: 'SUPPRESSION', name: 'Suppression Fire', description: 'Low Dmg + Weaken.', type: 'ATTACK', damageType: 'KINETIC', animation: 'RAILGUN', minLevel: 3, statusEffect: 'WEAKEN' },
  EXECUTE: { id: 'EXECUTE', name: 'Execute', description: 'Triple dmg if enemy < 30% HP.', type: 'ATTACK', damageType: 'KINETIC', animation: 'MELEE', minLevel: 8 },

  // Tank
  INTERCEPT: { id: 'INTERCEPT', name: 'Intercept', description: 'Protect ally next turn.', type: 'DEFENSE', animation: 'SHIELD', minLevel: 4 },
  SPIKE_ARMOR: { id: 'SPIKE_ARMOR', name: 'Spike Armor', description: 'Reflect Dmg (Passive buff).', type: 'DEFENSE', animation: 'MELEE', minLevel: 6 }, // Simulating passive via buff

  // Tech
  OVERLOAD: { id: 'OVERLOAD', name: 'Shield Overload', description: 'Consume Shield for AOE Dmg.', type: 'TECH', damageType: 'ELECTRIC', animation: 'EXPLOSION', minLevel: 5 },
  NANITE_CLOUD: { id: 'NANITE_CLOUD', name: 'Nanite Cloud', description: 'Heal Team over time.', type: 'SUPPORT', animation: 'REPAIR', minLevel: 6, statusEffect: 'REGEN_TEAM' }, // Using status logic
  INCINERATE: { id: 'INCINERATE', name: 'Incinerate', description: 'Apply Burn.', type: 'ATTACK', damageType: 'THERMAL', animation: 'LASER', minLevel: 4, statusEffect: 'BURN' },
};
...
export const getTerrainAt = (x: number, y: number, theme: string = 'TERRA'): TerrainType => {
  ...
  // Theme Overrides
  if (theme === 'INFERNO') {
      const volcanicHash = Math.abs(Math.sin(x * 0.05 + y * 0.05));
      if (volcanicHash > 0.6) { // Increased density
         if (hash > 0.90) return TerrainType.WALL;
         return TerrainType.MAGMA;
      }
  ...
  if (theme === 'GLITCH') {
      const glitchHash = Math.abs(Math.cos(x * 0.08 - y * 0.08));
      if (glitchHash > 0.7) {
         return TerrainType.GLITCH;
      }
      if (hash > 0.85) return TerrainType.WALL;
      // Random sporadic walls
      if (Math.random() > 0.95) return TerrainType.WALL;
      return TerrainType.FLOOR;
  }
  
  if (theme === 'XENON') {
      const bn = Math.cos(x * 0.1 + y * 0.1) * 123.45;
      const biomeHash = bn - Math.floor(bn);
      if (biomeHash < 0.6) { // High jungle density
         if (hash > 0.92) return TerrainType.WALL; 
         return TerrainType.JUNGLE;
      }
      if (hash < 0.1) return TerrainType.ACID_POOL; // More acid
      if (hash > 0.85) return TerrainType.WALL;
      return TerrainType.FLOOR;
  }
  ...
  // Phase 2 Biomes mixed in lightly
  const volcanicHash = Math.abs(Math.sin(x * 0.05 + y * 0.05));
  if (volcanicHash > 0.85) {
     if (hash > 0.85) return TerrainType.WALL;
     return TerrainType.MAGMA;
  }
...
export const AVAILABLE_MODULES: Module[] = [
  ...
  },
  {
    ...
    description: 'Immune to Magma Damage.',
    cost: 200,
    type: 'PASSIVE',
    effectId: 'THERMAL_PLATING',
    value: 0
  },
  {
    id: 'stability_anchor',
    name: 'Stability Anchor',
    description: 'Prevent Glitch teleportation.',
    cost: 200,
    type: 'PASSIVE',
    effectId: 'STABILITY_ANCHOR',
    value: 0
  }
];
...
export const generateSector = (sector: number): any => {
   // Theme Odds
   const rand = Math.random();
   let theme: 'TERRA' | 'INFERNO' | 'GLITCH' | 'XENON' = 'TERRA';
   let questType: 'REPAIR' | 'STABILIZE' | 'DEBUG' | 'HUNT' = 'REPAIR';
   let modifier: 'NONE' | 'HEATWAVE' | 'PACKET_LOSS' | 'SPORE_CLOUD' = 'NONE';
   let name = 'Terra Prime';
   let questTarget = 1;
   ...
   if (rand > 0.75) {
       ...
       questType = 'STABILIZE';
       modifier = 'HEATWAVE';
       name = `Proxima Centauri ${sector} (Volcanic)`;
       questTarget = 3; // 3 turns to stabilize
   } else if (rand > 0.5) {
       theme = 'GLITCH';
       questType = 'DEBUG';
       modifier = 'PACKET_LOSS';
       name = `Simulacrum ${sector} (Glitch)`;
       questTarget = 4; // 4 nodes
   } else if (rand > 0.25) {
       theme = 'XENON';
       questType = 'HUNT';
       modifier = 'SPORE_CLOUD';
       name = `Xenon Hive ${sector}`;
       questTarget = 3; // 3 bosses
   } else {
       name = `Terra Nova ${sector}`;
   }
