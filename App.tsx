...
import { INITIAL_PLAYER_STATE, ENCOUNTER_CHANCE_BASE, getTerrainAt, getPOIAt, BOT_CLASSES, ITEMS_DB, POD_POS, GUARDIAN_POS, BOSS_ENEMY, UNIQUE_NAMES, PERSONALITIES, SKILLS_DB, ACID_DAMAGE, DAMAGE_TYPE_CHART, NPC_TUTORIALS, SCORE_WEIGHTS, generateSector } from './constants';
import GameView from './components/GameView';
import CombatInterface from './components/CombatInterface';
import ExplorationInterface from './components/ExplorationInterface';

const createEnemy = (playerLevel: number, terrain: TerrainType): Enemy => {
  const r = Math.random();
  
  // Biome Specifics
  if (terrain === TerrainType.JUNGLE) {
      if (r > 0.6) return { type: 'STEALTH_SPIDER', class: 'ASSAULT', name: 'Stealth Spider', hp: 60 + (playerLevel*8), maxHp: 60 + (playerLevel*8), xpValue: 45 };
  }
  if (terrain === TerrainType.CRYSTAL) {
      if (r > 0.6) return { type: 'CRYSTAL_CRAB', class: 'TANK', name: 'Crystal Crab', hp: 100 + (playerLevel*12), maxHp: 100 + (playerLevel*12), xpValue: 60 };
  }
  if (terrain === TerrainType.MAGMA) {
      if (r > 0.6) return { type: 'PYRO_MECH', class: 'TANK', name: 'Pyro Mech', hp: 90 + (playerLevel*10), maxHp: 90 + (playerLevel*10), xpValue: 55 };
  }
  if (terrain === TerrainType.GLITCH) {
      if (r > 0.6) return { type: 'PHANTOM_BOT', class: 'TECH', name: 'Phantom Bot', hp: 50 + (playerLevel*5), maxHp: 50 + (playerLevel*5), xpValue: 50 };
  }
...
const App: React.FC = () => {
  ...
  const executeMoveStep = useCallback((targetPos: Position) => {
      setPlayer(prev => {
          ...
          if (dx > 0) facing = 'RIGHT';
           ...
           const terrain = getTerrainAt(targetPos.x, targetPos.y, prev.planetConfig.theme);
           ...
           // Planet Modifiers
           if (prev.planetConfig.modifier === 'HEATWAVE') {
               // Slower Shield regen? Or random burn?
               // For MVP simply trigger burn sometimes
               if (Math.random() < 0.05) {
                   const active = nextTeam[prev.activeSlot];
                   if (active.hp > 0) {
                      active.hp -= 5;
                      showMessage("HEATWAVE: Shield Overheat!");
                   }
               }
           }
           ...
           if (terrain === TerrainType.ACID_POOL) {
            ...
            if (active.hp > 0 && !active.isDefeated) {
             ...
             const active = nextTeam[prev.activeSlot];
             if (active.hp > 0 && !active.isDefeated && active.hp < active.maxHp) {
                active.hp = Math.min(active.maxHp, active.hp + 5);
             }
          }
          else if (terrain === TerrainType.MAGMA) {
             const active = nextTeam[prev.activeSlot];
             const hasPlating = active.modules.some(m => m.effectId === 'THERMAL_PLATING');
             if (!hasPlating && active.hp > 0 && !active.isDefeated) {
                active.hp = Math.max(0, active.hp - 20);
                if (active.hp === 0) {
                  active.isDefeated = true;
                  showMessage("MELTDOWN: Unit destroyed by magma.");
                  stopMovement();
                } else {
                  showMessage("WARNING: Extreme Heat (-20 HP).");
                }
             }
          }
          else if (terrain === TerrainType.GLITCH) {
             const active = nextTeam[prev.activeSlot];
             const hasAnchor = active.modules.some(m => m.effectId === 'STABILITY_ANCHOR');
             if (!hasAnchor && Math.random() < 0.3) {
                 // Teleport Logic
                 const rx = Math.floor(Math.random() * 5) - 2;
                 const ry = Math.floor(Math.random() * 5) - 2;
                 targetPos = { x: targetPos.x + rx, y: targetPos.y + ry };
                 showMessage("SYSTEM ERROR: Position Glitch!");
                 // Ensure we don't teleport into wall check happens next frame implicitly or we accept risk
             }
          }
          ...
          if (currentBot.hp > 0 && Math.random() < chance) {
            stopMovement();
            const newEnemy = createEnemy(currentBot.level, terrain);
            setEnemy(newEnemy);
  ...
  const handleInteract = () => {
    ...
    if (x === POD_POS.x && y === POD_POS.y) {
       // SECTOR 1 / REPAIR QUEST
       if (player.planetConfig.questType === 'REPAIR') {
           if (player.quest.stage === 'FIND_POD') {
              ...
              if (player.quest.hasOmniTool) {
                 // Trigger Hyperspace instead of Victory
                 setGameState(GameState.HYPERSPACE);
       ...
       // ENDLESS MODE QUESTS
           ...
           if (player.planetConfig.questProgress >= player.planetConfig.questTarget) {
               setGameState(GameState.HYPERSPACE);
               updateStats(s => ({ questsCompleted: s.questsCompleted + 1 }));
           } else {
               showMessage(`Mission Incomplete. Objective: ${player.planetConfig.questProgress}/${player.planetConfig.questTarget}`);
           }
       }
       return;
    }
    ...
    if (targetPOI === POIType.CACHE) {
      // DEBUG QUEST
      if (player.planetConfig.questType === 'DEBUG') {
           setPlayer(prev => ({
             ...prev,
             planetConfig: { ...prev.planetConfig, questProgress: prev.planetConfig.questProgress + 1 },
             visitedPOIs: { ...prev.visitedPOIs, [targetKey]: true }
           }));
           updateStats(s => ({ questsCompleted: s.questsCompleted + 1 }));
           showMessage(`Node Debugged: ${player.planetConfig.questProgress + 1}/${player.planetConfig.questTarget}`);
           return;
      }
      ...
      if (isQuestPart) {
          ...
          updateStats(s => ({ scrapsCollected: s.scrapsCollected + 10 })); 
      } else {
      ...
      // STABILIZE QUEST
      if (player.planetConfig.questType === 'STABILIZE') {
           setPlayer(prev => ({
             ...prev,
             planetConfig: { ...prev.planetConfig, questProgress: prev.planetConfig.questProgress + 1 },
             visitedPOIs: { ...prev.visitedPOIs, [targetKey]: true }
           ...
           updateStats(s => ({ questsCompleted: s.questsCompleted + 1 }));
           showMessage(`Vent Stabilized: ${player.planetConfig.questProgress + 1}/${player.planetConfig.questTarget}`);
           return;
      }
  ...
  const handleHyperspaceJump = () => {
       const nextSector = player.currentSector + 1;
       const nextConfig = generateSector(nextSector);
       
       setPlayer(p => ({
         ...p,
         currentSector: nextSector,
         planetConfig: nextConfig,
         pos: { x: 0, y: 0 },
         visitedPOIs: {},
         quest: { 
             stage: 'FIND_POD', 
             hasOmniTool: false, 
             clearedPOIs: [],
             // We reuse QuestState mostly, but specific logic for WIN condition varies
         },
         // Keep Inventory, Team, Scrap, Stats
       }));
       setGameState(GameState.EXPLORING);
       setLogs([]);
       addLog(`WARP SUCCESSFUL. Welcom to ${nextConfig.name}.`, 'gain');
       addLog(`MISSION: ${nextConfig.questType} - Target: ${nextConfig.questTarget}`, 'info');
       setIntroShown(false);
  };

  const calculateEnemyScale = (baseHp: number) => {
     // Scale by sector
     return Math.floor(baseHp * (1 + (player.currentSector * 0.2)));
  };

  const createScaledEnemy = () => {
     // ... use existing createEnemy but wrap or modify args?
     // Actually createEnemy takes level. We can pass artificial level boost.
     // Better to modify spawn logic inline or update createEnemy signature.
     // For now, let's just use player level factor which naturally scales as player does.
     // But we should boost it further to ensure challenge outpaces player.
     // Let's rely on player level for now + modifier.
  };
  ...
  // Helper for applying status
  const applyStatus = (bot: Bot | Enemy, effect: string, duration: number) => {
      // Simple implementation: Players track via 'statuses' prop, Enemies via distinct state or simplified
      // For MVP expansion, let's focus on Player dealing statuses to Enemy and vice-versa
      // We need to update Enemy type to hold statuses if we want full persistence, 
      // but for now we can track critical ones in combat state or add to Enemy interface.
      // Let's assume we added statuses to Enemy interface or use a local state.
      // To keep it simple for this step: We will process immediate effects here or add simple tracking.
  };
  ...
  const combatRound = async (action: string) => {
    ...
    // Check for Stun (Player)
    const playerStunned = (player.team[player.activeSlot].statuses?.['STUN'] || 0) > 0;
    if (playerStunned) {
        addLog("UNIT STUNNED! Turn skipped.", 'danger');
      ...
      if (skill) {
        ...
        if (skill.type === 'ATTACK' || skill.type === 'TECH') {
           let base = 12 + (currentBot.level * 3);
           
           // Skill Specifics
           if (skill.id === 'LASER_SHOT') base += 3;
           if (skill.id === 'BURST_FIRE') base = 10 + Math.random()*10 + (currentBot.level*3);
           if (skill.id === 'SNIPE') base = 35 + (currentBot.level*5);
           if (skill.id === 'EXECUTE') {
              if (enemy.hp < enemy.maxHp * 0.3) {
                  base = base * 3;
                  addLog("EXECUTE! Critical Damage!", 'gain');
              }
           }
           if (skill.id === 'FLANKING_SHOT') base += 10; // Simplified synergy for now
           
           dmg = Math.floor(base + dmgMod);
           ...
           if (skill.statusEffect) {
               // In a full implementation, we'd apply to enemy.statuses
               // For now, let's simulate immediate impact or text
               addLog(`Applied ${skill.statusEffect}!`, 'gain');
               if (skill.statusEffect === 'WEAKEN') dmg += 5; // Bonus for now
           }

           // Overload Synergy
           if (skill.id === 'OVERLOAD') {
               const shield = currentBot.tempShield || 0;
               if (shield > 0) {
                   dmg += shield * 2;
                   updateActiveBot(b => ({ ...b, tempShield: 0 }));
                   addLog("SHIELD OVERLOADED! Massive Damage!", 'gain');
               }
           }
           ...
           if (dmg > 0) {
             ...
             // ... existing defense logic + new skills
             if (skill.id === 'INTERCEPT') {
                 addLog("Intercept stance active.", 'gain');
                 // Logic would go in Enemy Phase
             }
            ...
            if (skill.id === 'NANITE_CLOUD') {
                 setPlayer(p => ({
                     ...p,
                     team: p.team.map(b => !b.isDefeated ? { ...b, hp: Math.min(b.maxHp, b.hp + 15) } : b)
                 }));
                 addLog("Nanite Cloud: Team healed +15.", 'gain');
            }
    ...
    setEnemy(prevEnemy => {
       ...
       if (prevEnemy.hp <= 0) {
        ...
        // HUNT QUEST LOGIC
        if (player.planetConfig.questType === 'HUNT') {
             // For now count any kill, or maybe check level?
             // Let's count any kill for MVP flow
             const newProgress = player.planetConfig.questProgress + 1;
             setPlayer(p => ({
                 ...p,
                 planetConfig: { ...p.planetConfig, questProgress: newProgress }
             }));
             addLog(`Hostile Eliminated. Progress: ${newProgress}/${player.planetConfig.questTarget}`, 'gain');
             if (newProgress >= player.planetConfig.questTarget) {
                 addLog("MISSION COMPLETE. RETURN TO POD.", 'gain');
                 player.quest.stage = 'COMPLETED'; // Force stage if needed
             }
        }
    ...
    <div className="fixed inset-0 bg-black flex flex-col items-center justify-center overflow-hidden font-vt323">
      <div className="w-full max-w-6xl h-full flex flex-col md:flex-row md:h-[90vh] gap-4 p-4">
        ...
        <div className="w-full md:flex-[2] md:w-auto flex flex-col justify-center relative min-h-0 shrink-0 aspect-[4/3] md:aspect-auto">
          {gameState === GameState.HYPERSPACE ? (
             <div className="w-full h-full flex flex-col items-center justify-center bg-indigo-950 border-4 border-cyan-500 rounded-lg p-8 text-center overflow-y-auto custom-scrollbar">
                <h1 className="text-6xl text-cyan-300 font-bold mb-4">SECTOR {player.currentSector} CLEARED</h1>
                <div className="text-2xl text-white mb-8">HYPERSPACE RELAY ACTIVE</div>
                
                <div className="flex flex-col gap-4 items-center">
                    <p className="text-slate-300">Repairs completed. Fueling engines...</p>
                    <button 
                      onClick={handleHyperspaceJump}
                      className="mt-8 px-8 py-4 bg-cyan-600 border-2 border-cyan-400 text-white text-xl hover:bg-cyan-500 animate-pulse"
                    >
                      MAXIMUM WARP TO SECTOR {player.currentSector + 1}
                    </button>
                    
                    <button 
                       onClick={() => window.location.reload()}
                       className="mt-4 px-4 py-2 text-slate-500 hover:text-white"
                    >
                       (Retire to Base - Reset Game)
                    </button>
                </div>
             </div>
