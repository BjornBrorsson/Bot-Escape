  ...
  return (
    <div className="relative w-full aspect-[4/3] border-4 border-slate-700 bg-black rounded-lg shadow-2xl overflow-hidden touch-none">
      ...
      {gameState === GameState.EXPLORING && (
         <>
            <div className="absolute top-2 left-2 pointer-events-none text-white font-mono text-sm bg-black/50 p-2 rounded border border-cyan-800 backdrop-blur-sm">
                <div className="text-cyan-400 font-bold uppercase mb-1">
                    SECTOR {player.currentSector}: {player.planetConfig.name}
                </div>
                <div className="text-white/90">
                    MISSION: {player.planetConfig.questType}
                    <div className="text-xs text-yellow-400 mt-1">
                        PROGRESS: {player.planetConfig.questProgress} / {player.planetConfig.questTarget}
                    </div>
                </div>
                {player.planetConfig.modifier !== 'NONE' && (
                    <div className="text-red-400 text-xs mt-1 animate-pulse">
                        WARNING: {player.planetConfig.modifier} DETECTED
                    </div>
                )}
            </div>
            ...
            <div className="absolute bottom-2 left-1/2 -translate-x-1/2 pointer-events-none text-[10px] text-white/30 uppercase tracking-widest">
                Tap Tile to Move • {player.quest.stage === 'FIND_POD' ? 'Locate Escape Pod' : player.quest.stage}
            </div>
         </>
      )}
    </div>
